...
모델 저장소와 양자화 파일은 계속 수정된다. 아래 크기는 2026-08-25에 확인한 대표값이며, 다운로드 직전 반드시 Hugging Face에서 **정확한 파일명, shard 수, 총크기, 라이선스, revision, 런타임 호환성**을 다시 확인한다.
...
2026-08-25 기준으로 이 절의 전용 모델 구성을 재확인했다. XiYanSQL-QwenCoder는 2504가 여전히 최신 공개판이고(2505·2506 등 후속 버전 자체가 미출시), OmniSQL과 Arctic-Text2SQL-R1도 후속 릴리스 없이 유지되고 있어 아래 선택표는 그대로 유효하다.
...
Moirai 2.0 R small의 가중치는 2026-08-25 기준 **CC-BY-NC-4.0(비상업)**이다. 비상업적 라이선스가 적용되는 배포는 상업 시스템에 사용하지 않는다.
...
![그림: 안전한 Text-to-SQL 파이프라인 — 자연어 질문을 명세화(모호하면 질문 되묻기)하고 스키마를 검색해 SQL 후보를 생성한 뒤, 구문·allowlist 검사, 읽기 전용 정책 검사, 실행 계획·비용 게이트를 차례로 통과해야 read-only 실행과 결과 검증에 도달한다. 실행 게이트 이전 단계는 DB에 닿지 않으며, 각 게이트의 실패는 제한된 repair loop(최대 3회)로 SQL 후보 생성에 되돌아가고, 제한을 초과하면 사람에게 이관한다.](../../assets/sql-gate-pipeline.svg)
