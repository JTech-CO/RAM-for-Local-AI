...
![그림: MoE에서 전체 전문가 가중치(전체 저장 파라미터, NNB)는 격자 전체로서 GGUF 파일 크기와 필요 메모리를 결정하고, 토큰 1개는 라우터를 거쳐 활성 전문가 일부만 통과하므로 생성 속도·계산량은 활성 파라미터(ANB)가 좌우한다는 개념도](../../assets/moe-total-vs-active.svg)
